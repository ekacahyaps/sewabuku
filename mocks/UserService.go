...
// Update provides a mock function with given fields: token, updateData, profilePhoto
func (_m *UserService) Update(token interface{}, updateData user.Core, profilePhoto *multipart.FileHeader) (user.Core, error) {
	ret := _m.Called(token, updateData, profilePhoto)

	var r0 user.Core
	if rf, ok := ret.Get(0).(func(interface{}, user.Core, *multipart.FileHeader) user.Core); ok {
		r0 = rf(token, updateData, profilePhoto)
	} else {
		r0 = ret.Get(0).(user.Core)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(interface{}, user.Core, *multipart.FileHeader) error); ok {
		r1 = rf(token, updateData, profilePhoto)
	} else {
		r1 = ret.Error(1)
	}
